...
async def cmd_start(message: Message, command: CommandStart):
    args = command.args  # Здесь берем аргументы

    if args == "buy":
        from handlers.user.purchase import buy_entry
        await buy_entry(message)
        return
    ...
    caption=(
        ...
        f"ℹ️ В случае проблем с ботом нажмите кнопку <b>🆘 Поддержка</b> и опишите проблему."
    ),
